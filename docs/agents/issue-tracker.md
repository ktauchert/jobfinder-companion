...
`ktauchert/jobfinder-companion`. Use the `gh` CLI for all operations.
...
`area:ingestion`, `area:types`, `area:ai`, `area:docs`, `area:infra`) and one
`type:*` label (`type:feature`, `type:bug`, `type:chore`, `type:research`).
Add `needs-design` when the issue should go through `/grill-with-docs` before
implementation. Triage labels are mapped in `triage-labels.md`.
