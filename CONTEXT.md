...
| **Must-have**       | A skill in the profile that should appear in a job. **Hard-filters** when the list is non-empty (job must match at least one); also drives `mustHaveCoverage` in ranking. See ADR 0006 and `docs/SEARCH-AND-EMBEDDINGS.md`.                                                     |
